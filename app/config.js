...
module.exports = function(config) {
	...
	var base = {
		...
		app: config.app,
		env: {
			keychain: config.keychainPassword,
		...
		crypto: {
			...
			salt: '48hge89rg498re4g965-sha1', // app salt CHANGE ME PLEASE
	...
	var loadConfig  = {
		...
		soapstone: config.path.app + '/config/soapstone.js'
	};
	for (var i in loadConfig) {
		try {
			base[i] = require(loadConfig[i])(config);
		} catch (e) {
			console.log('config failed to load', i, loadConfig[i], e);
		}
	}
